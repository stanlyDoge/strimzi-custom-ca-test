...
#cfssl sign -config config.json -profile CA -ca intermediate.pem -ca-key intermediate-key.pem strimzi.csr strimzi.json | cfssljson -bare strimzi
cfssl sign -config config.json -profile server -ca intermediate.pem -ca-key intermediate-key.pem strimzi.csr strimzi.json | cfssljson -bare strimzi
cfssl genkey clients.json | cfssljson -bare clients
cfssl sign -config config.json -profile CA -ca intermediate.pem -ca-key intermediate-key.pem clients.csr clients.json | cfssljson -bare clients
